...
// These are parameterised queries note @named parameters. 
// Input parameters are parsed and set before queries are executed 
const SQL_INSERT = 'INSERT INTO dbo.AppUser (First_Name, Last_Name, Email, Password, Role) VALUES (@firstName, @lastName, @email, @password, @role); SELECT * from dbo.AppUser WHERE User_Id = SCOPE_IDENTITY();'; 
...
router.post('/', async (req, res) => { 
    ...
    const first_Name = validator.escape(req.body.firstName); 
    if (first_Name === "") { 
        errors+= "invalid first Name; "; 
    } 

    // Escape text and potentially bad characters
    const last_Name = validator.escape(req.body.lastName); 
    if (last_Name === "") { 
        errors+= "invalid last Name; "; 
    } 
