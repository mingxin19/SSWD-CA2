...
const SQL_SELECT_ALL = 'SELECT * FROM dbo.Type ORDER BY Type_Name ASC for json path;';

// for json path, without_array_wrapper - use for single json result
const SQL_SELECT_BY_ID = 'SELECT * FROM dbo.Type WHERE Type_Id = @id for json path, without_array_wrapper;';


// GET listing of all types and returns JSON, http://server:port/type
...
// GET a type by id, id passed as parameter via url and returns JSON, http://server:port/type/:id
router.get('/:id', async (req, res) => {
    ...
    // returns a type with matching id
    try {
        // Get a DB connection and execute SQL
        const pool = await dbConnPoolPromise
        const result = await pool.request()
